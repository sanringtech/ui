export const cliTranslations = {
  ...
  'cli.add.title': 'add',
  'cli.add.body':
    "Copies one or more components' source files into your project and installs any missing peer dependencies. Start with button when you want the smallest first install; components with extra package requirements install those dependencies automatically. Page-level templates use the block/ prefix (for example `sanring add block/login`) and pull in the components they are built from. Pass --dry-run to preview exactly which files would be created or overwritten without touching your filesystem, or --check to validate that every registry file the install would need can actually be fetched, without writing anything. Use --diff to see the line-by-line changes against your local files before committing, or --view to print the raw registry content without writing anything.",
