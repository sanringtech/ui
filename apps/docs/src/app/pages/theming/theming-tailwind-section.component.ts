...
import { themingTailwindSource } from './theming.constants';

@Component({
  selector: 'app-theming-tailwind-section',
  standalone: true,
  ...
  template: `
    <app-component-page-section [section]="section()">
      ...
      <app-theming-code-panel label="styles.css" [code]="tailwindSource" language="css" />
      <p class="mt-4 text-sm text-[var(--docs-muted)]">{{ i18n.t('theming.tailwind.note') }}</p>
    </app-component-page-section>
  `,
})
export class ThemingTailwindSectionComponent {
  readonly section = input.required<ComponentPageSectionDefinition>();

  protected readonly i18n = inject(I18nService);
  protected readonly tailwindSource = themingTailwindSource;
}
