...
// Canonicalize and deduplicate peer dependency candidates from a single
// component's raw import scan. No transitive dedup against componentDeps
// upstreams — each component lists all peers it directly imports, matching
// the hand-written registry.json convention (re-declaring a peer already
// covered by an upstream componentDep is harmless: sanring add's
// collectPeerDeps performs a union across the full install set anyway).
export function canonicalizePeerDependencies(rawCandidates: string[]): string[] {
  return [...new Set(rawCandidates.map(canonicalizePackageName))].sort();
}
