...
@Directive({
  ...
  standalone: true,
  providers: [{ provide: SANRING_FIELD_CONTROL, useExisting: TextareaDirective }],
  host: {
    '[class]': 'hostClass',
    '[id]': 'id',
...
export class TextareaDirective implements SanringFieldControl<string>, DoCheck, OnDestroy {
  ...
  private previousState: TextareaStateSnapshot | null = null;
  ...
  private getStateSnapshot(): TextareaStateSnapshot {
    return {
      value: this.value,
...
interface TextareaStateSnapshot {
  value: string;
  focused: boolean;
  empty: boolean;
  errorState: boolean;
