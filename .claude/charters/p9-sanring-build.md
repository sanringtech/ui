...
- [ ] **unknown（重大，2026-08-11 發現）**：批次 B「peerDependency 遞移閉包去重」的整體設計前提有問題。批次 C 用全部 53 個真實元件（不是 spike 當時的 8 個抽樣）跑 golden fixture 回歸測試後發現：ADR-0001 spike 賴以推論去重規則的 `calendar` 案例是特例，不是慣例——至少 13 個元件（`checkbox`/`input`/`radio`/`select`/`combobox`/`otp-input`/`textarea`/`slider`/`switch`/`file-upload`/`pagination`/`alert-dialog`/`command`）的手寫 `registry.json` 即使 componentDeps 已經宣告過同一個 peer 套件，仍然照樣在自己身上重複宣告。傾向方案：整個拿掉 `computeUpstreamPeerCoverage`/`dedupePeerDependencies`，`sanring build` 改成單純列出掃描到的 baseline-filtered peer 套件，不嘗試去重（重複宣告無害，`add.ts` 的 `collectPeerDeps` 本來就是遞移合併）。但這改動會推翻已經 commit 的批次 B 程式碼跟其 golden fixture 測試核心論據，需要使用者拍板後才能動手，不可逕自腦補改掉。詳細追蹤見 `TODOLIST.md` 的 P9/P24 段落。`commands/build.test.ts` 的 golden fixture 測試目前用 `it.skip` 卡住，恢復條件是這個問題解決之後。
