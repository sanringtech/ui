# audit-component — Sanring 元件品質審查

元件開發完成後執行品質審查。檢查 Angular 結構正確性、a11y 合規、props/API 設計，以及是否有 spec 檔案——沒有的話自動補寫最低 baseline spec。

**用法**：`/audit-component button` 或 `/audit-component packages/ui/src/lib/components/switch`

此 skill 可獨立執行，也可由 `insert-component` 在 sync 前自動呼叫。
...
### 3-D registry.json 對應（元件須已在 registry.json 中）
...
寫完 spec 後，執行：
```bash
pnpm ng test "@sanring/ui" --include="**/<name>.spec.ts" --watch=false
```
...
```
## audit-component 審查結果：<component-name>
