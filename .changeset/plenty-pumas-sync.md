...
Also consolidates the duplicated `console.error` + `process.exit(1)` error handling in `registry.ts` into a single `die()` helper — no behavior change, just removes duplication that made the error paths awkward to test.
