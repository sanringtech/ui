...
1. **Stabilize the existing library before expanding the catalog.**
   Sanring UI already has 51 formal registry components. The next mainstream-library milestone is not raw component count; it is reliability, a11y, docs, tests, and API consistency.
...
Sanring UI currently has **51 formal registry components**:
...
- `registry`, `packages/ui`, docs pages, and `public-api.ts` are aligned for all 51 components.
...
`accordion`, `tabs`, `table`, `carousel`, `resizable`, `sidebar`, `avatar`, `breadcrumb`, `card`, `alert`, `badge`, `progress`, `skeleton`, `spinner`, `tag`, `timeline`, `tree`, plus the remaining display primitives.
